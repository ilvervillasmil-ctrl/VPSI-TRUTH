...
class TestEngineArranque:
    ...
    def test_roles_sin_duplicar(self):
        eng = _engine(verificar_axiomas=False, strict=False)
        nombres_por_rol = []
        for rol, lista in eng.registro.por_rol.items():
            for c in lista:
                nombres_por_rol.append((rol, c.nombre))
        # un nombre de módulo no puede ocupar dos roles distintos con el mismo contenedor
...
class TestEvaluar:
    ...
    def test_engine_no_inventa_sin_factores(self):
        ...
        # sin C/L/K y sin CA útil → PARCIAL, no inventa Tru
