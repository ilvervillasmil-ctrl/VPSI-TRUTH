...
5. [Módulos](#5-módulos)
   - [5.1. `core/engine.py`](#51-coreenginepy---autoridad-de-despacho)
   - [5.2. `modules/axiomas/`](#52-modulesaxiomas---juez-de-contraste)
   - [5.3. `modules/constante/`](#53-modulesconstante---el-ancla)
   - [5.4. `modules/formulas/`](#54-modulesformulas---la-ecuación)
   - [5.5. `modules/realidad/`](#55-modulesrealidad---canal-de-evidencia)
   - [5.6. `modules/correlacion_mecanica/`](#56-modulescorrelacion_mecanica---secuencia-y-acoplamiento)
   - [5.7. `modules/verificacion/`](#57-modulesverificacion---auto-auditoría)
   - [5.8. `modules/calculador/`](#58-modulescalculador---pendiente)
   - [5.9. `modules/contexto/`](#59-modulescontexto---pendiente)
   - [5.10. `modules/taxonomia/`](#510-modulestaxonomia---pendiente)
   - [5.11. `modules/cache/`](#511-modulescache---pendiente)
   - [5.12. `core/centinela.py`](#512-corecentinelapy---pendiente)
   - [5.13. `core/diagnostico.py`](#513-corediagnosticopy---huérfano)
...
## **1. Propósito** <a name="1-propósito"></a>
...
## **2. Principio de Arquitectura** <a name="2-principio-de-arquitectura"></a>
...
## **3. Estructura del Repositorio** <a name="3-estructura-del-repositorio"></a>
...
│   │   └── __init__.py      # Detecta contradicciones en secuencias
│   │
│   ├── calculador/          # CA: Produce C, L, K            [PENDIENTE]
│   │   └── __init__.py      # Fórmulas: C = 1 - k/m, L = 1 - r/p, K = 1 - f/c
│   │
│   ├── contexto/            # CX: Lógica del contexto        [PENDIENTE]
│   │   └── __init__.py      # Define conteos (k, m, r, p, f, c)
│   │
│   ├── taxonomia/           # TX: Tácticas T1–T15           [PENDIENTE]
│   │   └── __init__.py      # Describe, no puntúa
│   │
│   └── cache/               # Registro forense             [PENDIENTE]
│       └── __init__.py      # Guarda aprobar/rechazar con trazabilidad
│
├── diagnostics/
...
## **4. Mecanismo de Funcionamiento** <a name="4-mecanismo-de-funcionamiento"></a>

---

### **4.1. Dos Secuencias Distintas** <a name="41-dos-secuencias-distintas"></a>
...
- **`CX`**: Contexto (define conteos `k`, `m`, `r`, `p`, `f`, `c`).
...
### **4.2. Los Tres Filtros** <a name="42-los-tres-filtros"></a>
...
**Regla:**
- **Sólo cuando el centinela aprueba, hay salida.**
...
### **4.3. Lo Indefinido no es Cero** <a name="43-lo-indefinido-no-es-cero"></a>
...
- **El cálculo mide correspondencia; la táctica mide la maniobra.**
  - Escapar de una **mete en la otra**.

---

---

## **5. Módulos** <a name="5-módulos"></a>

---

### **5.1. `core/engine.py` — Autoridad de Despacho** <a name="51-coreenginepy---autoridad-de-despacho"></a>
**Estado:** ✅ **MONTADO**
...
### **5.2. `modules/axiomas/` — AX: Juez de Contraste** <a name="52-modulesaxiomas---juez-de-contraste"></a>
**Estado:** ✅ **MONTADO**
...
> ⚠️ **Nota:** `correlacion.py` contiene **causalidad**, no correlación. El nombre no refleja su contenido. Un `git mv` a `causal.py` no tendría efectos funcionales.

---

### **5.3. `modules/constante/` — CT: El Ancla** <a name="53-modulesconstante---el-ancla"></a>
**Estado:** ✅ **MONTADO**
...
### **5.4. `modules/formulas/` — FO: La Ecuación** <a name="54-modulesformulas---la-ecuación"></a>
**Estado:** ✅ **MONTADO**
...
### **5.5. `modules/realidad/` — Canal de Evidencia** <a name="55-modulesrealidad---canal-de-evidencia"></a>
...
### **5.6. `modules/correlacion_mecanica/` — Secuencia y Acoplamiento** <a name="56-modulescorrelacion_mecanica---secuencia-y-acoplamiento"></a>
**Estado:** ❌ **ROL NO ADMITIDO**

- **Función:** Lee los archivos de la carpeta en su orden nativo, calcula la mecánica que resulta de lo que ellos mismos dicen, y comprueba que no se contradigan.
...
### **5.7. `modules/verificacion/` — VX: Auto-Auditoría** <a name="57-modulesverificacion---auto-auditoría"></a>
**Estado:** ❌ **ROL NO ADMITIDO**
...
### **5.8. `modules/calculador/` — CA: Produce C, L, K** <a name="58-modulescalculador---pendiente"></a>
...
### **5.9. `modules/contexto/` — CX: Lógica del Contexto** <a name="59-modulescontexto---pendiente"></a>
**Estado:** ❌ **PENDIENTE**
...
### **5.10. `modules/taxonomia/` — TX: Tácticas T1–T15** <a name="510-modulestaxonomia---pendiente"></a>
**Estado:** ❌ **PENDIENTE**

- **Función:** **Describe, no puntúa**.
  - Las 15 tácticas **T1–T15** (no hay declaraciones hoy).
- **Lista de tácticas:**
...
### **5.11. `modules/cache/` — Registro Forense** <a name="511-modulescache---pendiente"></a>
**Estado:** ❌ **PENDIENTE**
...
### **5.12. `core/centinela.py` — Filtro de Orquestación** <a name="512-corecentinelapy---pendiente"></a>
**Estado:** ❌ **PENDIENTE**
...
### **5.13. `core/diagnostico.py` — Tabla de Códigos** <a name="513-corediagnosticopy---huérfano"></a>
**Estado:** ❌ **HUÉRFANO**
...
---
---
## **6. Fórmulas Canónicas** <a name="6-fórmulas-canónicas"></a>
...
---
---
## **7. Axiomas del Documento no Cargados** <a name="7-axiomas-del-documento-no-cargados"></a>
...
---
---
## **8. Problemas Abiertos** <a name="8-problemas-abiertos"></a>
...
---
---
## **9. Prioridad de Construcción** <a name="9-prioridad-de-construcción"></a>
...
---
---
## **10. Convenciones** <a name="10-convenciones"></a>
...
3. **Los filtros informan, no lanzan:**
   ...
   - **Sólo el arranque del `Engine` aborta** ante contradicción.
...
- El test de `evaluar()` **no exige que funcione**:
  ...
  - **Sólo falla si devuelve un número sin haber calculado nada.**

---
---
## **13. Diagrama de Flujo** <a name="13-diagrama-de-flujo"></a>

```mermaid
graph TD
    %% Estructura del sistema VPSI-TRUTH
    ...
    subgraph Leyenda
        direction TB
        O[Módulos Montados]:::montado
        P[Módulos Pendientes]:::pendiente
        Q[Módulos No Admitidos]:::no_admitido
    end
